...
## UI 실플레이 검증에서 잡은 것 (2026-07-05)
사용자 "직접 플레이해보며 이상한 점을 찾아봐" → 브라우저에서 실제 클릭 플레이.
- 🔴 **카드 이중 전송 버그**: 손패 카드 `onClick`이 wrapper `<span>`과 안쪽 `CardChip` 양쪽에 걸려, 클릭 시 버블링으로 `play-card`가 2번 전송 → 2번째가 "card not in hand"로 거절되어 **매 플레이마다 에러 배너**. CardChip의 중복 onClick 제거로 수정. 회귀 테스트 추가. (엔진 자가플레이·프로토콜 e2e는 각 액션 1회만 보내서 원리상 못 잡던 버그 — 실 UI 플레이로만 노출.)
- 화면 제목 "Mission"(영어)→"미션" 통일.
- 🔴 **봇이 게임을 즉시 터뜨림(핵심 "이상함")**: 규칙 버그 아님(spurious loss 0). 기본 봇이 **자기 태스크 카드를 못 이길 트릭에 버리고, 남의 태스크 카드를 덥석 획득** → 잘못된 소유자 획득으로 **전체 게임의 71%가 2트릭 내 즉사**. 손패 진단(zzz.lossdiag)으로 정량화.
  - **봇 방어적 개선**: 모든 태스크가 공개정보임을 이용해 — (1) 트릭에 남의 태스크가 있으면 안 이김, (2) 못 이길 때 자기 태스크 안 버림, (3) 마지막 차례+이기면 자기 태스크 획득, (4) 리드 시 무적 자기 태스크(색9/로켓4) 내서 완수. → **즉사율 71%→38%**, 중앙값 트릭수 1~3 → 7~13으로 대폭 상승. 실 UI에서 13트릭 완주 + 미션 성공 확인.
- 결과: **191 테스트 통과**(engine 110/shared 1/server 43/client 37), typecheck 0. 봇 안전플레이 테스트 + 손실정당성(모든 패배는 규칙상 원인 존재) 불변식 추가.
- 남음: 봇은 여전히 대부분 패배(협력 승률 낮음) — "확실히 이기는 봇"은 룩어헤드/조율이 필요한 별도 AI 과제.
