...
## 후속 백로그 (비차단)
- 비주얼 스킨(우주 테마) — 의도적 후순위. 디자인 전담 위임 예정.
- 룰 UI 잔여: 통신 토큰 위치 표시, 조난 카드전달, 커맨더 결정/분배 질의, dead-zone 직관.
- 역할 바인딩(M46 pink9holder / M50 협상 / M5·M33 sick·chosen) 셋업 흐름 + M48 마지막트릭 런타임 카드바인딩.
- 미션 데이터 order 토큰 ~15미션 보강(제약·taskCount는 정확).
- 서버: started/full 방 join nack, 방코드 충돌(극단값), 재접속 정교화, 인터넷 호스팅 배포.
- 정리: withHands 데드코드, 계획1 산문 정정.
